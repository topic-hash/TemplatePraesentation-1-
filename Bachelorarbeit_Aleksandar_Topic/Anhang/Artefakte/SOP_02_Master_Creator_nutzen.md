...
- Zeigt Bestätigung: "[OK] Created material: [Name]"
...
AI: [ENTITY_TYPE: material]
    ...
    [JSON Block]
    [OK] Created material: Titanium Dioxide
    [OK] Created material: PEG 400
    [OK] Created material: Tego
```
...
- Zeigt: "[AUTO] Auto-created material: blue-pigment-paste"
- Zeigt: "[OK] Created product: Blue Pigment Paste"
...
- Zeigt: "[CONFIG] Auto-corrected baseQuantity from [falsch] to [richtig]" (falls AI falsch rechnet)
...
- Fehlermeldung bei fehlenden Abhängigkeiten:
  ```
  [FEHLER] Recipe cannot be created. Missing:
  - BOM: blue-pigment-paste-bom
...
**Vorgehen:**
1. Auf Button "[NOTIZ] Chat zusammenfassen" klicken
...
**Beispiel:**
```json
// [FEHLER] GRACE-Export (fehlerhaft für Neuimport)
{
  "material": {"version": 6, "id": "peg-400"}
}

// [OK] Master Creator Output (korrekt)
{
...
2. Button "[SPEICHERN] Download [Entity-Type]" klicken
...
1. Button "[SPEICHERN] Download All" klicken
...
[WARNUNG] **Manuelle Nacharbeit (1-2h):**
...
- [ ] Integrierte Validierung durchgeführt ([OK] grün)
...
- [OK] 6 JSON-Dateien (einzeln exportiert)
- [OK] Chat-Zusammenfassung (Markdown)
- [OK] Validierungsprotokoll (grün = OK)
...
1. User: "Ich brauche Grey Pigment, Titanium Dioxide, PEG 400, Tego"
   -> [OK] 4 Materials erstellt

2. User: "Wir haben 1 Dissolver, 1 Pump, 1 Mixer"
   -> [OK] 3 Machines erstellt

3. User: "Wir produzieren Purple Pigment Paste"
   -> [OK] Product + Material auto-erstellt

4. User: "Prozess: Dosing 5min, Mixing 15min, Dispersion 10min, Filtration 20min"
   -> [OK] Process mit 4 Steps erstellt (Zeit in Sekunden)

5. User: "BOM für Purple Pigment Paste: Grey Pigment 10g, Titanium Dioxide 60g, PEG 400 10g, Tego 20g"
   -> [OK] BOM erstellt, baseQuantity auto-korrigiert auf 100g

6. User: "Erstelle Recipe für Purple Pigment Paste"
   -> [OK] Recipe erstellt (nutzt Session-Context)

7. Klick auf "[OK] Validieren" -> Alle OK
8. Klick auf "[NOTIZ] Chat zusammenfassen" -> Zusammenfassung generiert
9. Klick auf "[SPEICHERN] Download All" -> aps-all-{timestamp}.json
```
